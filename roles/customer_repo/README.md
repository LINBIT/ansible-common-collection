...
| `customer_repo_install_proxy_license` | `false` | Fetch `/etc/drbd-proxy.license` after registration. Interactive runs prompt with a y/N when not set in inventory. No-op when the contract has no DRBD Proxy seat |
| `customer_repo_proxy_license_path` | `/etc/drbd-proxy.license` | Destination path for the fetched DRBD Proxy license file |
| `customer_repo_manage_node_script_url` | `https://my.linbit.com/linbit-manage-node.py` | URL of the upstream `linbit-manage-node.py` helper script |
