...
| `customer_repo_staging` | `false` | Additionally write a second repo file (`linbit-staging.repo` on RPM distros, `linbit-staging.sources` on Debian/Ubuntu) containing staging versions of repos matching `customer_repo_staging_repos`. The production repo file is left unchanged. Staging repos are additive and partial — they typically ship only a small subset of packages under active testing, not a full 1:1 mirror of prod — so the production repo must stay enabled for everything else to resolve. RPM staging sections are suffixed with `-staging` and use `repo_gpgcheck=0` since staging repo metadata is not signed consistently; APT staging sets `Trusted: yes` for the same reason. |
| `customer_repo_staging_repos` | `['drbd-9']` | List of glob patterns (anchored, no implicit substring match). Repos matching any pattern are written to the staging file when `customer_repo_staging` is `true`. This list is authoritative for the staging file: `customer_repo_default_excludes` and `customer_repo_user_excludes` do NOT apply to staging, so a repo that is excluded in production can still be pulled from staging if added here |
| `customer_repo_install_proxy_license` | `false` | Fetch `/etc/drbd-proxy.license` via `linbit.common.linbit_proxy_license`. Skipped silently unless `linbit_cluster_id` is also set. Interactive runs prompt with a y/N when unset |
